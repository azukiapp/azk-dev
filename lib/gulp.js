...
AzkGulp.prototype = {
  ...
  init_builds: function() {
    ...
    var add_build = function(name, build_dir) {
      ...
      self.new_task(babel_task, [babel_clean_task], function () {
        ...
        var presents = self.config.babel.presets || [];
        self.config.babel.presets = presents.map(function(preset) {
        ...
        var plugins = self.config.babel.plugins || [];
        self.config.babel.plugins = plugins.map(function(plugin) {
        ...
        return self.gulp.src(build_dir.src + '/**/*.js', src_opts)
          ...
          .pipe(self.sourcemaps.write({ sourceRoot: self.sourcemaps_path + name }))
    ...
    self.new_task('babel:core:version', function() {
      self.gutil.log('babel-core version: ' + version());
    });

    // Adding deprecate option
    self._babel_runtime(version);

    // babel polyfill
    self.new_task('babel:polyfill:install', function() {
  ...
  _babel_runtime: function(version_fn) {
    var self = this;
    var optional = this.config.babel.optional;
    ...
    if (optional instanceof Array) {
      ...
      if (runtime) {
        ...
        self.new_task('babel:runtime:version', function() {
          self.gutil.log('babel version: ' + version_fn());
        });
